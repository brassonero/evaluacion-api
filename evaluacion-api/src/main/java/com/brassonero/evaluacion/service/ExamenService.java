...
import com.brassonero.evaluacion.exception.ExamNotFoundException;
import com.brassonero.evaluacion.exception.OptionNotFoundException;
import com.brassonero.evaluacion.exception.StudentNotFoundException;
import com.brassonero.evaluacion.model.dto.*;
import com.brassonero.evaluacion.model.entity.*;
import com.brassonero.evaluacion.model.repository.*;
import lombok.AllArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.stereotype.Service;

import static com.brassonero.evaluacion.constants.Constants.*;
import static com.brassonero.evaluacion.util.Util.convertToExamenEntity;
...
public class ExamenService implements IExamenService {
    ...
    public AsignacionResponse asignarExamen(Long idEstudiante, Long idExamen) {
        ...
        return AsignacionResponse.builder()
                .id(savedAsignacion.getId())
                .examen(ExamenResponse.builder()
                        .id(savedAsignacion.getExamen().getId())
                        .fechaExamen(savedAsignacion.getExamen().getFechaExamen())
                        .build())
                .estudiante(EstudianteResponse.builder()
                        .id(savedAsignacion.getExamen().getId())
                        .nombre(savedAsignacion.getEstudiante().getNombre())
                        .build())
                .build();
    }
    ...
    public void guardarRespuestas(Long idEstudiante, Long idExamen, List<Long> idOpciones) {
        ...
        idOpciones.forEach(idOpcion -> {
            ...
            if (!opcion.getPregunta().getExamen().getId().equals(idExamen)) {
                log.error("El idOpcion [{}] no pertenece al idExamen [{}]", idOpcion, idExamen);
